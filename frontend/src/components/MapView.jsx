...
import { RAMP_GREEN } from '../lib/colorRamps'
...
// Naturalist green ramp
const COLOR_RANGE = RAMP_GREEN
...
function valueToColor(value, allData) {
  if (!allData?.length || !value) return [0, 0, 0, 0]
  const max = Math.max(...allData.map(d => d.value))
  const t   = Math.pow(value / max, 0.4)
  const i   = Math.min(Math.floor(t * (COLOR_RANGE.length - 1)), COLOR_RANGE.length - 1)
  const alpha = Math.floor(40 + t * 240)  // 40..240: transparente en bajos, sólido en altos
  return [...COLOR_RANGE[i], alpha]
}
...
// ── Styles ─────────────────────────────────────────────────────────────────
const s = {
  controls:  { position:'absolute', top:0, left:0, right:0, zIndex:10, display:'flex', alignItems:'center', gap:8, padding:'6px 16px', background:'var(--topbar-bg)', borderBottom:'1px solid var(--border-2)', height:44 },
  ...
  tooltip:      { position:'absolute', bottom:60, left:20, zIndex:20, background:'rgba(237,235,229,0.95)', border:'1px solid var(--border-2)', padding:'10px 14px', pointerEvents:'none' },
  ...
  legend:      { position:'absolute', bottom:20, right:20, zIndex:10, background:'rgba(237,235,229,0.9)', border:'1px solid var(--border-2)', padding:'10px 14px' },
  ...
  legendBar:   { height:6, width:120, background:'linear-gradient(to right, #e4e8c8, #b4d9b3, #7aaa6e, #2d5c3f)', borderRadius:1 },
  legendLabels:{ display:'flex', justifyContent:'space-between', width:120, marginTop:4, fontSize:10, color:'var(--text-3)' },

  cellCount: { position:'absolute', bottom:20, left:20, zIndex:10, fontSize:10, color:'var(--text-3)', background:'rgba(237,235,229,0.9)', border:'1px solid var(--border-2)', padding:'5px 10px' },

  warnOverlay: { position:'absolute', inset:0, zIndex:50, background:'rgba(0,0,0,0.3)', display:'flex', alignItems:'center', justifyContent:'center' },
  warnBox:     { background:'var(--topbar-bg)', border:'1px solid var(--border-2)', padding:'24px 28px', maxWidth:400 },
